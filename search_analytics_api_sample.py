    ...
    for uri in property_uri_list:
      if isinstance(request, list):
        for requestdata in request:
          response = execute_request(service, uri.value, requestdata)
          time.sleep(2)
          row2 = print_table(response, uri.value, form, requestdata)
          print_row_array.append(row2)
      else:
        response = execute_request(service, uri.value, request)
        time.sleep(2)
        row2 = print_table(response, uri.value, form, request)
        print_row_array.append(row2) 
  else:
    if isinstance(request, list):
      for requestdata in request:
        response = execute_request(service, form['property_uri'].value, requestdata)
        time.sleep(2)
        print_row_array.append(print_table(response, form['property_uri'].value, form, requestdata))
    else:
      response = execute_request(service, form['property_uri'].value, request)
      time.sleep(2)
      print_row_array.append(print_table(response, form['property_uri'].value, form, request))
...
def get_csv():
  ...
  with open('sample_search_console.csv', encoding='utf-8') as csvfile:
